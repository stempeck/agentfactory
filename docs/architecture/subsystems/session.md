...
| _(removed)_ | _(was `BEADS_DIR`)_ | Store directory is now resolved via `config.StoreDir(factoryRoot)` in `mcpstore/lifecycle.go:147`; see `.designs/worktree-isolation/constraint-verification.md:5-6` (store directory is anchored to the ORIGINAL factory-root `.agentfactory/store/`, never a worktree). | One issuestore per factory, shared across worktrees. Worktree agents must still write to the same DB. |
...
- **Factory-root store directory regardless of worktree.** The issuestore resolves its path via `config.StoreDir(factoryRoot)` — `factoryRoot`, not `worktreePath`. One issuestore per factory.
