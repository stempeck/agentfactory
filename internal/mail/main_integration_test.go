...
// package's tests unchanged. The mail router integration test reaches the real tmux
// socket, so it must be guarded too. GuardCIOnly is a no-op in CI / a clean checkout.
// The default-suite TestMain in main_test.go is //go:build !integration, so this
