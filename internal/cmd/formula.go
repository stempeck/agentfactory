...
func runFormulaAgentGen(cmd *cobra.Command, args []string) error {
	...
	// Write CLAUDE.md. The render above stays local: the freshly generated .md.tmpl is not in this
	// binary's embed.FS, so templates.RenderIdentity would resolve the new role to the manager or
	// supervisor fallback. Only the write is shared.
	if err := templates.WriteIdentity(wsDir, []byte(content)); err != nil {
		return err
	}
...
// The output uses {{ .Role }}, {{ .RootDir }}, {{ .WorkDir }} for variable content.
// Formula-specific content — the description included — is baked in as literal text.
func generateAgentTemplate(f *formula.Formula, agentName string, agentType string) string {
	...
	// Identity (matches supervisor.md.tmpl structure). The description is BAKED, not left as
	// {{ .Description }}: agents.json stores only the first sentence (see the truncation below),
	// so every renderer except agent-gen used to shrink a generated agent's identity to one line.
	// {{ .Description }} survives in manager.md.tmpl and supervisor.md.tmpl, which have no formula.
	b.WriteString("# Agent Identity: {{ .Role }}\n\n")
	// Trim trailing whitespace before the period so a multi-paragraph description (which ends with a
	// newline) does not land the "." on its own line, and skip the period when the text already ends
	// in terminal punctuation to avoid a doubled mark (#681 T13).
	desc := strings.TrimRight(f.Description, " \t\r\n")
	period := "."
	if strings.HasSuffix(desc, ".") || strings.HasSuffix(desc, "!") || strings.HasSuffix(desc, "?") ||
		strings.HasSuffix(desc, ":") || strings.HasSuffix(desc, ";") {
		period = ""
	}
	b.WriteString("You are **{{ .Role }}**, " + escapeTmplDelimiters(desc) + period + "\n\n")
	...
	b.WriteString("\n## Startup Protocol\n\n")
	// The SessionStart hook array already delivers mail before this text is read, so step 1 acts on
	// what is in hand rather than ordering a second fetch — matching the reworded startup directive
	// (prime.go outputStartupDirective) so the instruction surface says one thing (#681 T11).
	b.WriteString("1. Act on the mail delivered at session start (`af mail inbox` lists ids for `af mail delete`)\n")
